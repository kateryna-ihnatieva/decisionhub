...
binary_relations_bp = Blueprint('binary_relations', __name__, url_prefix='/binary-relations')
...
def names():
    ...
    binary_task = request.args.get('binary_task') if request.args.get('binary_task') else None
...
def result(method_id=None):
    ...
    if matr == 0:
        ...
        add_object_to_db(db, BinaryMatrix, id=new_record_id, binary_names_id=new_record_id, matrix=matrix)
    else:
    ...
    sorted_dict = dict(sorted(sum_dict.items(), key=operator.itemgetter(1), reverse=True))
    ...
    if existing_record is None:
        add_object_to_db(db, BinaryRanj, id=new_record_id,
    ...
    if existing_record is None:
        add_object_to_db(db, BinaryTransitivity, id=new_record_id,
                         binary_names_id=new_record_id,
                         binary_matrix_id=new_record_id,
        ...
        if current_user.is_authenticated:
            add_object_to_db(db, Result, method_name='Binary',
    ...
    if pdf_request:
        return generate_pdf(context, 'Binary/result.html')
