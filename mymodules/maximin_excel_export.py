#!/usr/bin/env python3
"""
Excel export functionality for Maximin method
"""
...
class MaximinExcelExporter:
    ...
    def create_general_info_sheet(self, analysis_data):
        ...
        self.set_data_style(ws["B5"], analysis_data.get("matrix_type", "N/A"))

        # Task description
        task_description = analysis_data.get("maximin_task", "Немає опису завдання")
    ...
    def create_cost_matrix_sheet(self, analysis_data):
        ...
        # Min column
        min_col = chr(66 + len(name_conditions))
        self.set_subheader_style(ws[f"{min_col}3"], "Мін")
        ...
        for i in range(len(name_alternatives)):
            ...
            if i < len(min_values):
                ...
                self.set_number_style(ws[f"{min_col}{row}"], int(min_value), 0)
            else:
                self.set_number_style(ws[f"{min_col}{row}"], 0, 0)
    ...
    def create_optimal_variants_sheet(self, analysis_data):
        ...
        # Table headers
        self.set_subheader_style(ws["A3"], "Альтернативи")
        self.set_subheader_style(ws["B3"], "Мінімальне значення")

        name_alternatives = analysis_data.get("name_alternatives", [])
        min_values = analysis_data.get("min_values", [])

        if not name_alternatives or not min_values:
            self.set_data_style(ws["A5"], "Немає даних для оптимальних варіантів")
            return

        # Create ranking data
        ranking_data = []
        for i, (alternative, min_value) in enumerate(
            zip(name_alternatives, min_values)
        ):
            ranking_data.append((alternative, min_value))

        # Sort by min value in descending order
        ranking_data.sort(key=lambda x: x[1], reverse=True)

        # Fill ranking table
        for i, (alternative, min_value) in enumerate(ranking_data):
            row = i + 5
            self.set_data_style(ws[f"A{row}"], alternative)
            self.set_number_style(ws[f"B{row}"], int(min_value), 0)
    ...
    def create_ranking_chart(self, analysis_data):
        ...
        # Header
        self.set_header_style(ws["A1"], "Графік оптимальних варіантів")
        ws.merge_cells("A1:D1")
        ...
        chart.type = "col"
        chart.style = 10
        chart.title = "Мінімальні значення альтернатив"
    ...
    def generate_maximin_analysis_excel(self, analysis_data):
        """Generate complete maximin analysis Excel file"""
